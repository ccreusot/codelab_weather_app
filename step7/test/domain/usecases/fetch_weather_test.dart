...
void main() {
  ...
  final useCase = FetchWeather(FetchLocation(locationRepository),
      FetchWeatherFromLocation(repository), FetchWeatherFromCity(repository));

  test(
      'We want to fetch the weather from our location but we don\'t have the permission',
      () async {
    final expected = null;

    when(locationRepository.getLocation()).thenAnswer((realInvocation) async =>
        LocationStatus.error("You don't have the permission"));
  ...
  test('We want to fetch the weather from our location but an error occured',
    ...
    when(locationRepository.getLocation()).thenAnswer((realInvocation) async =>
        LocationStatus.success(Location(0.32, -0.3444)));
