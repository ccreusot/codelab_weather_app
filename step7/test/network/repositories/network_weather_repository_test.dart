...
void main() {
  ...
  test(
    ...
    when(service.getLocalWeather(any, any))
        .thenThrow(ClientException("Connection error"));

    expect(await repository.getFromLocation(Location(0.32, -2.100)),
        WeatherStatus.error("Connection error"));
  });
  ...
  test(
    ...
    expect(await repository.getFromLocation(Location(0.32, -2.100)),
        WeatherStatus.error("Something went wrong."));
  });
  ...
  test(
    ...
    when(service.getLocalWeather(any, any)).thenAnswer((realInvocation) async =>
        NetworkWeather.Weather.fromJson(
  ...
  test(
    ...
    when(service.getCityWeather(any))
        .thenThrow(ClientException("Connection error"));

    expect(await repository.getFromCity("paris"),
        WeatherStatus.error("Connection error"));
  });

  test(
      'If there is any other error, we want to get the weather from a city, we should receive a WeatherStatus.error',
      () async {
    final repository = NetworkWeatherRepository(service);
  ...
  test(
    ...
    final weather = Weather(
        ...
        <DailyForecast>[
          DailyForecast("14/11/2020", 16, 10,
              "https://prevision-meteo.ch/style/images/icon/ciel-voile-big.png"),
          DailyForecast("15/11/2020", 15, 9,
              "https://prevision-meteo.ch/style/images/icon/eclaircies-big.png"),
          DailyForecast("16/11/2020", 12, 8,
              "https://prevision-meteo.ch/style/images/icon/eclaircies-big.png"),
          DailyForecast("17/11/2020", 11, 9,
              "https://prevision-meteo.ch/style/images/icon/eclaircies-big.png"),
        ]);
    final repository = NetworkWeatherRepository(service);

    when(service.getCityWeather(any)).thenAnswer((realInvocation) async =>
        NetworkWeather.Weather.fromJson(
            convert.jsonDecode(await file.readAsString())));

    expect(
        await repository.getFromCity("paris"), WeatherStatus.success(weather));
  });
}
