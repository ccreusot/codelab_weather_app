...
import 'package:complete_weather_app/network/network_service.dart';
import 'package:complete_weather_app/network/models/weather.dart' as NetworkWeather;
...
class NetworkWeatherRepository implements WeatherRepository {
  ...
  Future<WeatherStatus> getFromCity(String city) async {
    try {
      final NetworkWeather.Weather weather = await _service.getCityWeather(city);
  ...
  Future<WeatherStatus> getFromLocation(Location location) async {
    try {
      final NetworkWeather.Weather weather = await _service.getLocalWeather(location.latitude, location.longitude);
  ...
  WeatherSuccess _mapNetworkWeatherToDomainWeather(NetworkWeather.Weather weather, bool geolocalized) {
    return WeatherStatus.success(Weather(
        ...
        weather.fcstDay0.hourlyData.entries.map((entry) {
          return HourlyForecast(
              Jiffy(entry.key.replaceAll("H", ":"), "h:mm").dateTime, entry.value.icon, entry.value.tMP2m.toInt());
        }).toList(),
        <NetworkWeather.ForecastDay>[weather.fcstDay1, weather.fcstDay2, weather.fcstDay3, weather.fcstDay4]
            .map((forecast) =>
                DailyForecast(forecast.date.replaceAll('.', '/'), forecast.tmax, forecast.tmin, forecast.iconBig))
            .toList()));
  }
}
