...
void main() {
  ...
  test('We want to load the list of watched city, if it is empty it should return an empty list', () async {
    final useCase = LoadWatchedCities(repository);

    when(repository.loadCities()).thenAnswer((realInvocation) async => <City>[]);

    expect(await useCase(), []);
  });

  test('We want to load the list of watched city, if not empty it should return the list filled.', () async {
    final expected = <City>[City("Paris", "France", "paris")];
    final useCase = LoadWatchedCities(repository);

    when(repository.loadCities()).thenAnswer((realInvocation) async => expected);
